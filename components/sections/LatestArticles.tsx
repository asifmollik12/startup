          ...
          {articles.map((article) => (
            <Link key={article.id} href={`/articles/${article.slug}`}
              ...
              <div className="p-5">
                ...
                <h3 className="font-serif font-bold text-base text-brand-dark mt-1 mb-2 leading-snug group-hover:text-brand-red transition-colors line-clamp-2">
                  {article.title}
                </h3>
                ...
                <div className="flex items-center gap-2 text-xs text-gray-400 border-t border-brand-border pt-3">
                  ...
                  <span>·</span>
                  <span className="flex items-center gap-1"><Clock size={9} />{article.readTime}m</span>
                </div>
              </div>
            </Link>
          ))}
